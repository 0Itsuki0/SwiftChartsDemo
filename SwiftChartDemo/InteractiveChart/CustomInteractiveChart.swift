...
struct CustomInteractiveChartDemo: View {
        ...
        ScrollView {
            ...
            .chartGesture { proxy in
                DragGesture(minimumDistance: 0)
                    ...
                    // uncomment for reset the range selection onEnd
//                    .onEnded { _ in
//                        selectedDateRange = nil
//                        selectedValueRange = nil
//                    }
            }
            ...
            Text("Custom: Select only if Data Exist")
            ...
            .chartGesture { proxy in
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        ...
                        let threshold = 0.1
                        let thresholdX = proxy.plotSize.width * threshold
                        let thresholdY = proxy.plotSize.height * threshold
                        
                        for data in self.seriesDataList {
                            guard let position = proxy.position(for: (data.date, data.value)) else {continue}
                            if abs(location.x-position.x) <= thresholdX && abs(location.y-position.y) <= thresholdY {
                                proxy.selectXValue(at: position.x)
                                proxy.selectYValue(at: position.y)

                            }
                        }
                    }
                    // uncomment for reset the selection onEnd
//                    .onEnded { _ in
//                        selectedDate = nil
//                        selectedValue = nil
//                    }
